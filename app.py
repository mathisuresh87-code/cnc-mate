...
# 2. ROD & TUBE CALCULATOR
    ...
    if calc_mode == "Advanced Mode (Drawing Scan & 3D)":
        st.markdown('<div style="background: rgba(72, 202, 228, 0.1); padding: 15px; border-radius: 10px; border: 1px solid #48CAE4; margin-bottom: 15px;"><b>Advanced Mode Active:</b> Upload your part drawing / 2D blueprint. Dimensions will be auto-scanned!</div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        required_qty = st.number_input("Required Quantity (Nos)", min_value=0, value=0, step=1)
        cycle_sec = st.number_input("Cycle Time (Seconds)", min_value=0.0, value=0.0, step=0.5)
    ...
    if st.session_state.calc_results is not None:
        ...
        st.info(f"🔄 **Conversion Details:** Entered {rod_length_input} {res['unit_type']} | Equivalent Length: **{res['total_rod_meters']:.2f} Meters** | Equivalent Weight: **{res['equivalent_kg']:.2f} Kg**")
...
# 3. TRAUB COLLET & BAR FEED MASTER (With Step-by-Step Beginner Learning Guide)
    ...
    with t_col2:
        ...
        feeder_type = st.selectbox("Bar Feeder Type", ["Gravity / Weight Feed", "Pneumatic Push Feed", "Hydrobar / Magazine Bar Loader"])
