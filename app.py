...
# 2. ROD & TUBE CALCULATOR
  ...
  if st.button("Calculate & Render Part Preview"):
    ...
    total_days = (
        total_machine_time / shift_hours if shift_hours > 0 else 0.0
    )
    prod_per_shift = int(prod_per_hr * shift_hours)
  ...
  if st.session_state.calc_results is not None:
    ...
    t2.info(
        f"**Estimated Days ({res['shift_hours']} hrs/day):**"
        f" {res['total_days']:.2f} Days"
    )
    t3.info(
        f"**Production Rate:** {res['prod_per_hr']} parts/hr (~"
